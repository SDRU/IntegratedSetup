        ...
        if len(devices) == 0:
            ...
            for device in devices:
                ...
                if serial_nr.startswith("6"):
                    ...
                    self.Shutter.open() 
                    self.Shutter.set_operating_mode(mode=1) # manual mode
                    self.on = 1
                    print('Shutter initialized and open')
...
# close all devices in case of exception, only if they are open already
def close_devices(devices):
    for device in devices:
        if (device != None and device.on == 1):            
            device.close()  